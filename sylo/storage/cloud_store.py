"""Cloud storage backend for Sylo SDK.

Syncs execution data to the Sylo Cloud API via HTTP. This backend
...
logger = logging.getLogger("sylo.storage.cloud")
...
class CloudStorage(SyloStorage):
    """HTTP-based storage that syncs to the Sylo Cloud API.

    All requests include the API key in the Authorization header.
    Network failures are retried with exponential backoff.

    Args:
        api_key: Sylo Cloud API key (prefixed with "sylo_").
        base_url: Base URL for the Sylo Cloud API.
    """

    def __init__(
        self,
        api_key: str,
        base_url: str = "https://api.sylo.dev",
    ) -> None:
    ...
    async def _request(
        ...
        Raises:
            SyloStorageError: After all retries are exhausted.
        """
        import asyncio
    ...
    async def save_execution(self, record: ExecutionRecord) -> None:
        """Save an execution record to Sylo Cloud."""
    ...
    async def get_checkpoint(
        ...
        """Retrieve a checkpoint from Sylo Cloud."""
        try:
    ...
    async def list_executions(
        ...
        """List executions from Sylo Cloud."""
        try:
            data = await self._request(
                "GET",
    ...
    async def append_audit_event(
        ...
        """Append an audit event to Sylo Cloud."""
    ...
    async def save_approval_request(self, request: ApprovalRequest) -> None:
        """Save an approval request to Sylo Cloud."""
    ...
    async def get_approval_request(self, approval_id: str) -> ApprovalRequest | None:
        """Retrieve an approval request from Sylo Cloud by approval ID."""
        try:
    ...
    async def get_approval_request_by_step(
        ...
        """Retrieve an approval request from Sylo Cloud by step name."""
        try:
            data = await self._request(
